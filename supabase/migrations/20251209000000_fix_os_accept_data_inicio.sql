-- Fix: Corrigir os_accept para sempre garantir data_inicio >= data_abertura
-- Date: 2025-12-09
-- Issue: Violação da constraint ordens_servico_datas_logicas ao aceitar OS
-- Causa: COALESCE preservava data_inicio antiga que podia ser < data_abertura
...
BEGIN
  ...
  -- 10. Calcular data_inicio garantindo que SEMPRE seja >= data_abertura
  -- FIX: Não usar COALESCE que preservaria valor antigo inválido
  -- Usar o maior valor entre: now(), data_abertura, data_inicio existente
  v_data_inicio := GREATEST(
    now(),
    v_os.data_abertura,
    COALESCE(v_os.data_inicio, v_os.data_abertura)
  );

  -- 11. Atualizar OS - SEMPRE atualiza data_inicio para valor válido
  UPDATE ordens_servico
  SET 
    tecnico_id = v_tecnico_id,
    status = 'em_deslocamento',
    data_inicio = v_data_inicio,  -- Não usa COALESCE aqui
    updated_at = now()
  WHERE id = p_os_id;
...
COMMENT ON FUNCTION os_accept(uuid) IS 'Permite que um técnico aceite uma OS disponível e inicie o deslocamento. Garante que data_inicio sempre seja >= data_abertura usando GREATEST()';
